...
- 注销团队空间 Owner 时，系统按现有 Owner、Editor、Viewer 的顺序稳定选择空间接任者，并按 Owner、Admin、Member 的顺序独立选择组织接任者；转移、审计和注销在同一事务内完成。任一组织或团队空间没有接任者时返回 `422`，整次注销不产生变更。仅无其他活跃成员的个人空间会清理本地数据，共享团队空间及其模型配置会保留。
