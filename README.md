...
模型不是浏览、编辑、本地检索或分类的必要条件；AI 生成、补证、治理和投稿内容预审使用当前 Workspace 的个人模型配置。投稿预审每次执行时读取该 Workspace 的最新配置，但只发送脱敏后的不可变投稿快照，并使用平台统一、版本化的审核规则；provider 和 model 会进入审核记录，Base URL、API Key、私人分类体系和公共分类提案不会进入模型输入、日志或 Admin 响应。断网或模型失败不会阻塞原料入箱、正文阅读或人工分类。
