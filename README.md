...
- 公开协作：精确投稿快照、独立平台 AI 预审、管理员审核、发布负责人治理、复用许可、私人导入、订阅、结构化纠错、举报与审计。
...
模型不是浏览、编辑、本地检索或分类的必要条件；AI 生成、补证、治理和投稿内容预审需要有效的模型配置。分类和标签完全由用户在当前保存或投稿流程中选择或创建，不向模型发送私人分类体系或公共分类提案。断网或模型失败不会阻塞原料入箱、正文阅读或人工分类。
