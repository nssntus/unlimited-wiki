...
import { apiGet, apiPost, type Notification, queryKeys } from "@/lib/api"
...
export function NotificationsPage() {
  ...
  return <PageFrame><div className="mx-auto max-w-4xl"><PageTitle eyebrow="账号" title="通知" description="下架、重新上架和公开内容处理结果会保留在这里。" />
    {notifications.isLoading ? <Skeleton className="h-72" /> : notifications.data?.length ? <div className="divide-y border-y">{notifications.data.map((item) => {
      const publicTarget = item.object_type === "public_entry" && !["public_removed", "public_withdrawn"].includes(item.kind)
      return <article key={item.id} className="flex flex-col gap-3 py-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-medium">{item.title}</h2>{!item.read_at && <StatusBadge value="未读" kind="warn" />}</div><p className="mt-2 text-sm text-muted-foreground">{item.message}</p><p className="mt-2 text-xs text-muted-foreground">{new Date(item.created_at).toLocaleString()}</p></div><div className="flex flex-wrap gap-2">{publicTarget && <Button size="sm" variant="outline" render={<Link to={`/square/entries/${item.object_id}`} />}><ExternalLinkIcon data-icon="inline-start" />查看广场</Button>}{!item.read_at && <Button size="sm" onClick={() => markRead.mutate(item.id)}><CheckIcon data-icon="inline-start" />标为已读</Button>}</div></div></article>
