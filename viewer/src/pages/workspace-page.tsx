...
export function WorkspacePage() {
  ...
  return <PageFrame><div className="mx-auto max-w-4xl">
    ...
    <Dialog open={inviteOpen} onOpenChange={setInviteOpen}><DialogContent><DialogHeader><DialogTitle>邀请已有账号</DialogTitle><DialogDescription>这里仅授予已注册账号当前团队空间的权限。对方接受前不会获得任何空间权限。</DialogDescription></DialogHeader><FieldGroup><Field><FieldLabel htmlFor="invite-email">账号邮箱</FieldLabel><Input id="invite-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} /><FieldDescription>{session?.user?.role === "admin" ? <>对方还没有账号？先到 <Link to="/admin/accounts" onClick={() => setInviteOpen(false)}>账号邀请</Link> 创建注册链接。</> : "对方需要先联系平台管理员创建账号。"}</FieldDescription></Field><Field><FieldLabel>角色</FieldLabel><Select value={role} onValueChange={(value) => value && setRole(value as Role)}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value="editor">Editor · 可读写内容</SelectItem><SelectItem value="viewer">Viewer · 只读</SelectItem></SelectGroup></SelectContent></Select></Field>{invite.isError && <FieldError>{invite.error.message}</FieldError>}</FieldGroup><DialogFooter><Button variant="outline" onClick={() => setInviteOpen(false)}>取消</Button><Button disabled={!email.trim() || invite.isPending} onClick={() => invite.mutate()}>{invite.isPending && <Spinner data-icon="inline-start" />}发送邀请</Button></DialogFooter></DialogContent></Dialog>
