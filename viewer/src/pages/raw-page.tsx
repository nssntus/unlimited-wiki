...
export function RawPage() {
  ...
  return <PageFrame><article className="mx-auto max-w-[760px]"><PageTitle eyebrow={<StatusBadge value="Raw 只读快照" kind="warn" />} title={raw.data.title} description={<code className="break-all">{raw.data.path}</code>} actions={<Button render={<Link to={`/ingest/${raw.data.path}`} />}><InboxIcon data-icon="inline-start" />摄入</Button>} /><MarkdownContent markdown={raw.data.markdown.replace(/^#\s+.*\n/, "")} fromPath={raw.data.path} /></article></PageFrame>
}
